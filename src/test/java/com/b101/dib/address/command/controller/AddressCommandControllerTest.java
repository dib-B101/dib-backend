...
class AddressCommandControllerTest {
    ...
    @Test
    void updatesAddressOwnedByAuthenticatedMember() throws Exception {
        AccessTokenClaims claims = new AccessTokenClaims(1L, MemberRole.USER);
        ...
        given(accessTokenVerifier.verifyBearer("Bearer access-token")).willReturn(claims);
        given(addressCommandService.update(1L, 10L, request)).willReturn(response);

        mockMvc.perform(patch("/api/v1/members/me/addresses/10")
                        .header(HttpHeaders.AUTHORIZATION, "Bearer access-token")
                        .contentType(MediaType.APPLICATION_JSON)
                        .content("""
                                {
                                  "name":"새 회사",
                                  "apiAddressId":"new-address-api-id"
                                }
                                """))
                ...
                .andExpect(jsonPath("$.number").value("06236"))
                .andExpect(jsonPath("$.address").value("서울특별시 강남구 테헤란로"))
                .andExpect(jsonPath("$.name").value("새 회사"))
                .andExpect(jsonPath("$.apiAddressId").value("new-address-api-id"));

        verify(addressCommandService).update(1L, 10L, request);
    }
    ...
    @Test
    void rejectsAddressUpdateWithoutAccessToken() throws Exception {
        mockMvc.perform(patch("/api/v1/members/me/addresses/10")
                        .contentType(MediaType.APPLICATION_JSON)
                        .content("""
                                {
